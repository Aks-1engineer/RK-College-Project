...
if($row=mysqli_fetch_array($res))
{
   $curdate=$row['date'];
                 ...
                 $file_name=$_FILES['file']['name'];
                  ...
                  //echo $session_value;
                  include("connection.php");
                  $query2="insert into tbl_naac(session,file_name,type,added_on) value('$session','$file_name','$session_value',now())";
                  mysqli_query($con,$query2);
                  $location="naac/naac_file/";
                  move_uploaded_file($tmp_name,$location.$file_name);
                  header("location:naac_show.php?msg=1");
