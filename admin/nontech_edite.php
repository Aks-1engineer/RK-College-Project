        ...
        <div class="row mx-1 mt-1">
            ...
            <div class="col-sm-9 bg-white">
                <div class="row bg-white">
                 ...
                 //echo $non_id;
                 include("connection.php");
                 $query2="select * from tbl_nonteacher where non_id='$non_id'";
                 $res2=mysqli_query($con,$query2);
                 if($row2=mysqli_fetch_array($res2))
                 {
                 ?>
                         <div class="row">
                    <h3 class="text-center"><b><i>Update Non-teaching Staff</i></b></h3>
                           <div class="col-sm-6">
                           <form action="nontech_update.php" method="post" enctype="multipart/form-data">
                               <input type="hidden" name="non_id" value="<?php echo $row2['non_id']; ?>" />
                               Name
                               <input type="text" value="<?php echo $row2['name']; ?>" name="name" class="form-control mb-2">
   
                               Father's Name
                               <input type="text" value="<?php echo $row2['fname']; ?>" name="fname" class="form-control mb-2">
       
                         Date of Birth
                         <input type="text" value="<?php echo $row2['dob']; ?>" name="dob" class="form-control mb-2">
   
                               
                               Designation
                               <input type="text" value="<?php echo $row2['designation']; ?>" name="designation" class="form-control mb-3">
                             
                               Select Gender
                               <br>
                               
                               <input type="radio" name="gender" <?php if($row2['gender']=='male'){  ?> checked <?php } ?> value="male" class="ms-3">Male 
                               <input type="radio" name="gender" <?php if($row2['gender']=='female'){  ?> checked <?php } ?> value="female" class="ms-5">Female
                             
   
                           </div>
                           <div class="col-sm-6">
                               Department
                               <input type="text" value="<?php echo $row2['dept_name']; ?>" name="department" class="form-control mb-2">
                               Highest Qualification
                               <input type="text" value="<?php echo $row2['qualification']; ?>" name="qualification" class="form-control mb-2">
                               Mobile
                               <input type="number" value="<?php echo $row2['mobile']; ?>" name="mnumber" class="form-control mb-2">
                               Upload Pic
                               <input type="file" value="<?php echo $row2['pic']; ?>" name="pic" class="form-control mb-2">
                               
                               Email
                               <input type="email" value="<?php echo $row2['email']; ?>" name="email" class="form-control" >
                           ...
                           <div class="row">
                               <div class="col-sm-12 mt-2">
                                 Address
                                   <textarea name="address" id="" cols="30" rows="6" class="form-control  my-textarea" ><?php echo $row2['address']; ?></textarea>
                               </div>
                           </div>
